...
public class AgentActionExecutor {
    ...
    public AgentActionExecutor(TaskConversationStore conversationStore, WebReadOnlyToolService webReadOnlyToolService) {
    ...
    public String renderAvailableActions() {
        return """
                ...
                10. web_search
                    Purpose: Search the public web using a configured provider.
                    Input: {"query": "search terms", "maxResults": 5}
                    Uses BRAVE_SEARCH_API_KEY when configured; otherwise uses a limited DuckDuckGo Instant Answer fallback.
                """;
    }

    public ActionExecutionResult execute(ChatRequest chatRequest, AgentActionRequest actionRequest) {
        if (actionRequest == null || !actionRequest.isAction()) return ActionExecutionResult.failure("Invalid action request.");
        String action = clean(actionRequest.getAction());
        if (action.isBlank()) return ActionExecutionResult.failure("Action name is required.");
    ...
    private ActionExecutionResult rememberNote(ChatRequest chatRequest, Map<String, Object> input) {
        ...
        if (note.isBlank()) return ActionExecutionResult.failure("remember_note requires input.note.");
        if (note.length() > MAX_NOTE_LENGTH) return ActionExecutionResult.failure("remember_note input.note is too long. Keep notes under " + MAX_NOTE_LENGTH + " characters.");
    ...
    private ActionExecutionResult createTask(ChatRequest chatRequest, Map<String, Object> input) {
        ...
        if (name.isBlank()) return ActionExecutionResult.failure("create_task requires input.name.");
        if (name.length() > MAX_TASK_NAME_LENGTH) return ActionExecutionResult.failure("create_task input.name is too long. Keep names under " + MAX_TASK_NAME_LENGTH + " characters.");
        if (taskNameExists(chatRequest, name)) return ActionExecutionResult.failure("A task named \"" + name + "\" already exists. Choose a more specific name.");
    ...
    private ActionExecutionResult renameTask(ChatRequest chatRequest, Map<String, Object> input) {
        ...
        if (taskId == null) return ActionExecutionResult.failure("rename_task requires input.taskId when there is no current task.");
        if (!taskExists(chatRequest, taskId)) return ActionExecutionResult.failure("Cannot rename task " + taskId + " because it is not in the current task snapshot.");
        String name = clean(String.valueOf(input.getOrDefault("name", "")));
        if (name.isBlank()) return ActionExecutionResult.failure("rename_task requires input.name.");
        if (name.length() > MAX_TASK_NAME_LENGTH) return ActionExecutionResult.failure("rename_task input.name is too long. Keep names under " + MAX_TASK_NAME_LENGTH + " characters.");
        if (taskNameExistsForOtherTask(chatRequest, taskId, name)) return ActionExecutionResult.failure("Another task named \"" + name + "\" already exists. Choose a different name.");
    ...
    private ActionExecutionResult closeTask(ChatRequest chatRequest, Map<String, Object> input) {
        Integer taskId = integerInput(input, "taskId").orElse(chatRequest.getTaskId());
        if (taskId == null) return ActionExecutionResult.failure("close_task requires input.taskId when there is no current task.");
        if (!taskExists(chatRequest, taskId)) return ActionExecutionResult.failure("Cannot close task " + taskId + " because it is not in the current task snapshot.");
    ...
    private ActionExecutionResult listTasks(ChatRequest chatRequest) {
        ...
        if (tasks == null || tasks.isEmpty()) return ActionExecutionResult.success("No frontend tasks were provided in the request snapshot.");
        String rendered = tasks.stream()
                .map(task -> "- id=" + task.getId() + ", name=\"" + clean(task.getName()) + "\", status=" + clean(task.getStatus()) + ", lifecycle=" + clean(task.getLifecycle()) + ", parentTaskId=" + task.getParentTaskId() + ", createdBy=" + clean(task.getCreatedBy()))
    ...
    private ActionExecutionResult webFetchUrl(Map<String, Object> input) {
        ...
        } catch (Exception e) { return ActionExecutionResult.failure("web_fetch_url failed: " + e.getMessage()); }
    }

    private ActionExecutionResult webPageOutline(Map<String, Object> input) {
        try { return ActionExecutionResult.success(webReadOnlyToolService.webPageOutline(clean(String.valueOf(input.getOrDefault("url", ""))))); }
        catch (Exception e) { return ActionExecutionResult.failure("web_page_outline failed: " + e.getMessage()); }
    }
    ...
    private ActionExecutionResult webExtractLinks(Map<String, Object> input) {
        ...
        } catch (Exception e) { return ActionExecutionResult.failure("web_extract_links failed: " + e.getMessage()); }
    }
    ...
    private ActionExecutionResult webSearch(Map<String, Object> input) {
        try {
            ...
            return ActionExecutionResult.success(webReadOnlyToolService.webSearch(query, maxResults));
        } catch (Exception e) { return ActionExecutionResult.failure("web_search failed: " + e.getMessage()); }
    }

    private boolean taskExists(ChatRequest request, Integer taskId) { return request.getTasks() != null && taskId != null && request.getTasks().stream().anyMatch(task -> taskId.equals(task.getId())); }
    private boolean taskNameExists(ChatRequest request, String name) { return request.getTasks() != null && request.getTasks().stream().anyMatch(task -> clean(task.getName()).equalsIgnoreCase(name.trim())); }
    private boolean taskNameExistsForOtherTask(ChatRequest request, Integer taskId, String name) { return request.getTasks() != null && request.getTasks().stream().filter(task -> !taskId.equals(task.getId())).anyMatch(task -> clean(task.getName()).equalsIgnoreCase(name.trim())); }
    ...
    private Optional<Integer> integerInput(Map<String, Object> input, String key) {
        ...
        try { return Optional.of(Integer.parseInt(String.valueOf(value))); } catch (NumberFormatException e) { return Optional.empty(); }
    }
