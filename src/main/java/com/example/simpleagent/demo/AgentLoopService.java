...
public class AgentLoopService {
    ...
    public AgentLoopService(LlamaServerManager llamaServer, AgentActionExecutor actionExecutor, TaskConversationStore conversationStore) {
    ...
    public ChatResponse run(ChatRequest request) {
        ...
        try {
            ...
            for (int step = 1; step <= MAX_ACTION_STEPS; step++) {
                ...
                if (decision == null) return finish(request, rawModelOutput, clientTaskActions);
                if (decision.isFinal()) {
                    String finalAnswer = clean(decision.getContent());
                    if (finalAnswer.isBlank()) finalAnswer = "I completed the request, but the model returned an empty final answer.";
                ...
                if (decision.isAction()) {
                    ...
                    if (result.getClientTaskAction() != null) clientTaskActions.add(result.getClientTaskAction());
            ...
            return finish(request, "I hit the maximum number of action steps before finishing. Here is what happened:\n" + String.join("\n", actionTrace), clientTaskActions);
        } catch (Exception e) {
    ...
    private String renderContext(ChatRequest request) {
        if (request.getContext() == null || request.getContext().isEmpty()) return "(none)";
        StringBuilder sb = new StringBuilder();
        int step = 1;
        for (String line : request.getContext()) {
            if (line != null && !line.isBlank()) sb.append(step++).append(") ").append(line).append("\n");
        }
        return sb.toString().trim();
    }

    private String renderTaskSnapshot(ChatRequest request) {
        if (request.getTasks() == null || request.getTasks().isEmpty()) return "(none)";
        StringBuilder sb = new StringBuilder();
        for (TaskSnapshot task : request.getTasks()) {
            sb.append("- id=").append(task.getId()).append(", name=\"").append(clean(task.getName())).append("\", status=").append(clean(task.getStatus())).append(", lifecycle=").append(clean(task.getLifecycle())).append(", parentTaskId=").append(task.getParentTaskId()).append(", createdBy=").append(clean(task.getCreatedBy())).append("\n");
        }
        return sb.toString().trim();
    }
    ...
    private AgentActionRequest parseDecision(String rawModelOutput) {
        ...
        try { return objectMapper.readValue(json, AgentActionRequest.class); } catch (Exception ignored) { return null; }
    }
    ...
    private String formatActionTrace(int step, AgentActionRequest decision, ActionExecutionResult result) {
        return "step=" + step + ", action=" + decision.getAction() + ", success=" + result.isSuccess() + ", observation=" + result.getObservation();
    }

    private String cleanModelText(String value) {
        if (value == null) return "";
        String cleaned = value.replaceAll("(?is)<think>.*?</think>", "").trim();
        if (cleaned.startsWith("```json")) cleaned = cleaned.substring("```json".length()).trim();
        if (cleaned.startsWith("```")) cleaned = cleaned.substring("```".length()).trim();
