...
function normalizeMessage(message) {
  ...
  return {
    ...
    ...(Array.isArray(message?.modelCallTraces) ? { modelCallTraces: message.modelCallTraces } : {}), ...(Array.isArray(message?.webToolTraces) ? { webToolTraces: message.webToolTraces } : {}),
  }
}
    ...
    <div className="task-panel">
      ...
      <div className="messages" ref={messagesRef} onScroll={handleMessagesScroll}>
        ...
        {task.messages.map((msg, idx) => {
          ...
          const modelTraceCount = Array.isArray(msg.modelCallTraces) ? msg.modelCallTraces.length : 0; const webToolTraceCount = Array.isArray(msg.webToolTraces) ? msg.webToolTraces.length : 0; const hasTrace = msg.role === 'assistant' && (modelTraceCount > 0 || webToolTraceCount > 0); return (
            <div key={idx} className={`message message-${msg.role}`}>
              ...
              {hasTrace && (
                ...
                >
                  Deep dive: {modelTraceCount} model call{modelTraceCount === 1 ? '' : 's'}{webToolTraceCount > 0 ? ` | ${webToolTraceCount} tool action${webToolTraceCount === 1 ? '' : 's'}` : ''}
                </button>
              )}
            </div>
          )
  ...
  const sendMessage = useCallback(async (taskId, text, options = {}) => {
    ...
    try {
      ...
      const aiMessage = {
        ...
        durationMs,
        originalPrompt: text,
        modelCallTraces: Array.isArray(result.modelCallTraces) ? result.modelCallTraces : [], webToolTraces: Array.isArray(result.webToolTraces) ? result.webToolTraces : [],
      }
